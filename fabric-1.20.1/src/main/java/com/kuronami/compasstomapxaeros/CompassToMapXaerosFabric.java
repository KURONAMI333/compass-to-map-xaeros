...
/**
 * Compass to Map: Xaero's edition - Fabric 1.20.1 Mod entry (server / common 側)。
 *
 * <p>クライアント側の packet 受信ハンドラは {@link CompassToMapXaerosClientFabric}。
 * <p>1.20.1 では vanilla {@code CustomPacketPayload} がないため
 * {@code PayloadTypeRegistry} 経由の登録は行わず、
 * {@code FabricPacket} の {@code PacketType} のみで送受信する。
 */
public class CompassToMapXaerosFabric implements ModInitializer {

    @Override
    public void onInitialize() {
        // Config 登録 (FCAP v8 系: api.config.v2 パッケージ)
        ...
        CompassToMapXaeros.LOGGER.info("Compass to Map: Xaero's edition (Fabric 1.20.1) initialized");
    }
}
